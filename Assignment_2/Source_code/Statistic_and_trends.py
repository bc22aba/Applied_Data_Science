...
@author: cheru
"""
#Note: Please comment seaborn library if there is an issue "no module named SEABORN"
...
file_name = 'D:/WorkSpace/Applied_Data_Science/Assignment_2/Datasets/\
API_19_DS2_en_csv_v2_5361599.csv'
df = Read_CSV_File(file_name);
df.head(5)

cntry_list = ["CHN", "DEU", "FRA", "GBR", "JPN", "KOR", "MEX", "USA", "ZWE"]
indicator_list = ["AG.LND.FRST.ZS", "EG.ELC.COAL.ZS", "EN.ATM.CO2E.KT",\
                  "EN.ATM.METH.KT.CE", "SP.POP.GROW", "EG.ELC.NUCL.ZS",\
                      "SP.URB.TOTL.IN.ZS"]
df_cntry = df[df['Country Code'].isin(cntry_list)]
dfc_indicator = df_cntry[df['Indicator Code'].isin(indicator_list)]

data1 = dfc_indicator.drop(dfc_indicator.loc[:,'1960':'1989'].columns,\
                           axis = 1)

df_final = data1.loc[:,['Country Name','Indicator Name','1990','1995','2000',\
                        '2005','2010','2014']]
df_final[['1990','1995','2000','2005','2010','2014']] = df_final[['1990',\
                        '1995','2000','2005','2010','2014']].astype(float)

# Saving file with Years as Columns
df_final.to_csv('df_final.csv')

# shape from wide to long with melt function in pandas
df2=pd.melt(df_final,id_vars=['Country Name','Indicator Name'],\
            var_name='Year', value_name='values')

# reshape from long to wide in pandas python
df2=df2.pivot(index=['Indicator Name','Year'], columns='Country Name', \
              values='values')
df2.head(5)
...
df2_EP_COAL = df_final[df_final['Indicator Name']==\
                     'Electricity production from coal sources (% of total)']
df2EPCOAL = df2_EP_COAL.drop(['Indicator Name'], axis = 1)
df2EPCOALt = df2EPCOAL.transpose()
EPCOAL =df2EPCOALt.rename\
    (columns=df2EPCOALt.iloc[0]).drop(df2EPCOALt.index[0])
...
plt.xlabel('Time Line', fontsize=12)
plt.title('Electricity production from coal sources (from 1990 to 2014)',\
          fontsize=12)
plt.show()

# Methane gas emissions 
df2_Methane = df_final[df_final['Indicator Name']==\
                       'Methane emissions (kt of CO2 equivalent)']
df2Methane = df2_Methane.drop(['Indicator Name'], axis = 1)
df2Methanet = df2Methane.transpose()
Methane =df2Methanet.rename\
    (columns=df2Methanet.iloc[0]).drop(df2Methanet.index[0])
...
plt.xlabel('Time Line', fontsize=12)
plt.title('Methane emissions (kt of CO2 equivalent)(from 1990 to 2014)',\
          fontsize=12)
plt.show()

# Nuclear Power Production 
df2_Nuclear = df_final[df_final['Indicator Name']==\
                  'Electricity production from nuclear sources (% of total)']
df2Nuclear = df2_Nuclear.drop(['Indicator Name'], axis = 1)
df2Nucleart = df2Nuclear.transpose()
Nuclear =df2Nucleart.rename\
    (columns=df2Nucleart.iloc[0]).drop(df2Nucleart.index[0])
...
# Population Growth 
df2_UP = df_final[df_final['Indicator Name']==\
                  'Urban population (% of total population)']
df2UP = df2_UP.drop(['Indicator Name'], axis = 1)
df2UP['% change'] = ((df[['1990', '2014']].pct_change(axis=1)\
                      ['2014']) * 100).round(2).map(str) + '%'
